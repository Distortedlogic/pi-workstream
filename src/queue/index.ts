 ...
 *
 * Shortcuts:
 *   Ctrl+Shift+Q         → open queue editor overlay
 *
 * Tool:
...
import { type Focusable, type OverlayHandle, matchesKey, truncateToWidth } from "@earendil-works/pi-tui";
...
export default function (pi: ExtensionAPI) {
	...
	function renderWidgetLines(theme: Theme, width: number): string[] {
		...
		if (state.currentTask) {
			...
			lines.push(
				truncateToWidth(theme.fg("accent", "🎯 ") + multiIcon + theme.fg("toolTitle", flat) + pauseIcon, width),
			);
	...
	// ── Shortcut ──
	// Ctrl+Q stays assigned to the integrated Context Tree panel.

	pi.registerShortcut("ctrl+shift+q", {
		description: "Open workstream queue editor",
		handler: async (ctx) => {
	...
	pi.registerTool({
		...
		label: "Enqueue Task",
		description:
			"Add a task to the deferred task queue. Use only when the user explicitly asks you to queue or defer something for later.",
		...
		handler: async (args, ctx) => {
			...
			if (sub === "add") {
				...
				if (!result.added) {
					notify(
						ctx,
						result.reason === "active" ? "Already active." : `Already queued at #${result.position}.`,
						"warning",
					);
				} else {
